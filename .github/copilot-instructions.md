...
- **v0.3.1**: RELEASED (2026-09-15) — hotfix for two defects found while testing the published 0.3.0 against real servers: **(1)** exactly-once was violated with more than one worker (the claim was a read-only `FOR UPDATE SKIP LOCKED` poll; 2 workers × 40 tasks produced 45–78 executions on PostgreSQL **and** MySQL) and **(2)** MySQL/MariaDB workers stalled on **Python 3.11** (`Timed out waiting for a MySQL connection (30.0s)`, 0/40 completed in 4/4 runs) because `asyncmy`'s pool loses a wakeup when an acquisition waiter is cancelled. Schema stays **v6** (no migration).
...
### Codified decisions — v0.3.1 (atomic claiming + MySQL pool robustness)
- **Tasks are claimed, not polled.** The worker's poll path is `QueryBuilder.claim_pending_tasks()`; it moves the rows to `processing` **as part of taking the claim**. `select_pending_tasks()` is now a **read-only** query (dashboard/`list_pending_tasks` only) and must never be used to claim: with autocommit its `FOR UPDATE SKIP LOCKED` locks are gone when the statement ends, so two workers receive the same rows (the exact v0.1–v0.3.0 defect; regression guard = `tests/integration/test_backend_matrix.py::TestExactlyOnceClaim`).
- **PostgreSQL/SQLite** claim in one statement, but the locking read **must** live in a materialised CTE: `WITH claimed AS MATERIALIZED (SELECT t.task_id … ORDER BY … LIMIT $n FOR UPDATE SKIP LOCKED) UPDATE conductor_tasks SET status='processing', worker_id=$w, started_at=NOW() WHERE task_id IN (SELECT task_id FROM claimed) RETURNING *`. **Never move `FOR UPDATE SKIP LOCKED` into the UPDATE's subquery** — PostgreSQL then ignores the `LIMIT` for the rows it locks and updates (verified: `LIMIT 10` claimed all 40 pending rows and stranded 30).
- **Parameter order in that statement is textual, not logical**: the CTE precedes the `SET` clause, so the bound order is `route, limit, worker_id` (positional backends such as SQLite bind by position). Placeholders are rendered from a running counter rather than hardcoded numbers.
- **MySQL/MariaDB** (no `RETURNING`) claim inside `pool.transaction()`: locking `SELECT … FOR UPDATE SKIP LOCKED … LIMIT %s`, then `UPDATE … WHERE status IN ('pending','retrying') AND task_id IN (…)` (status re-checked so a concurrently completed row is never reported as claimed), then re-`SELECT` the rows by `worker_id`. `idx_tasks_polling (status, scheduled_for, priority DESC, created_at)` keeps the locking read index-driven.
- **Never claim more than the worker can start**: `_poll_and_execute` caps the batch at `concurrency − len(in_flight)`; a claimed-but-unstarted row is invisible to other workers. The circuit-breaker skip path **must** call `release_claim()` (a claim would otherwise stay `processing`).
- **Dead-worker recovery**: `reclaim_stale_tasks(stale_after)` returns rows to `pending` only when the claim is older than the window **and** the owner is not alive — a `conductor_workers` row with a fresh heartbeat. An owner with **no** heartbeat row counts as dead. The worker calls it throttled (once per window) with `Worker(stale_claim_timeout=…)`, default `max(3 × heartbeat_interval, 30s)`; do not make it unconditional (it would steal claims from slow-but-live workers).
- **MySQL pool acquisition is retried in slices** until the `pool_timeout` budget is spent (`MySqlPool._acquire_connection`). `asyncmy`'s pool can consume the wakeup of a released connection when the waiter is cancelled — reproduced on **CPython 3.11 only** (3.12+ is fine) with a Conductor-free reproducer, and it starves the pool while `freesize > 0`. Do not revert to a bare `asyncio.wait_for(self._pool.acquire(), self._timeout)`.
- **Verification bar for claiming**: unit tests pin the rendered SQL per dialect (`TestAtomicClaimRendering`), the matrix pins behaviour on every backend, and the live check is 2 worker **processes** × 40 tasks ⇒ exactly 40 executions.
